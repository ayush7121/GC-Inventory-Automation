...
if st.button("🚀 Generate Automated Report"):
    if not mapping_file or not monday_file:
        ...
        try:
            ...
            # --- THE DETECTIVE: Catch missing columns before they crash the app ---
            if 'SKU' not in map_df.columns or 'Category' not in map_df.columns:
                st.error(f"🚨 Master Map Error: I need columns named exactly 'SKU' and 'Category'. Instead, your Master Map has: {map_df.columns.tolist()}")
                st.stop()
                
            if 'SKU' not in monday_df.columns:
                st.error(f"🚨 Monday Export Error: I need a column named exactly 'SKU'. Instead, your Monday file has: {monday_df.columns.tolist()}")
                st.stop()
            # -----------------------------------------------------------------
            ...
            # FIX: Only calculate math for the mapped "Main SKUs" to match Anne's manual math.
            mapped_df = merged_df[merged_df['Category'] != 'Uncategorized'].copy()
            
            # Identify 'Glow' by checking the Inventory Name; default everything else to 'PL'
            mapped_df['Brand'] = np.where(mapped_df['Inventory Name'].str.contains('Glow', case=False, na=False), 'Glow ', 'PL')
            
            total_sku_all = mapped_df['SKU'].nunique()
            total_item_all = mapped_df['On Hand'].sum()
            
            summary_data = []
            for brand in ['PL', 'Glow ']:
                brand_df = mapped_df[mapped_df['Brand'] == brand]
