...
with tab1:
    ...
    with col2:
        raw_file = st.file_uploader("2. Upload Raw Monday Export (CSV)", type=["csv"], key="raw_upload")
...
with tab2:
    ...
    st.write("Drag and drop all your individual category files (e.g., Melts.csv, Tealight.csv) here. The system will automatically stitch them together into a perfect `master_map.csv`.")
    
    # Allow multiple files to be uploaded at once
    category_files = st.file_uploader("Upload Category CSVs", type=["csv"], accept_multiple_files=True, key="cat_upload")
        ...
        if category_files:
            ...
            with st.spinner("Extracting SKUs and building map..."):
                for file in category_files:
                    # Ignore the raw data files if accidentally uploaded
                    ...
                    try:
                        ...
                        if 'SKU' in df.columns:
                            ...
                            # Extract category from filename (e.g., 'GC Inventory - Melts.csv' -> 'Melts')
                            category_name = file.name.replace('.csv', '').split(' - ')[-1].strip()
            ...
            if all_maps:
                ...
                # Drop any duplicate SKUs just in case they overlap
                master_map = master_map.drop_duplicates(subset=['SKU'])
                
                # Convert to CSV memory buffer
            ...
            st.warning("Please upload at least one category CSV to build the map.")